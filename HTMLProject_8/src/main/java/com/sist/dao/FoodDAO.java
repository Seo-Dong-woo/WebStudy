...
public class FoodDAO {
	...
	private static FoodDAO dao; // 클라이언트 한 명당 한 개만 사용 가능
	// 싱글턴 => 메모리 누수 방식 => DAO
	// 미리 연결된 주소값 얻기
	// 메소드는 한 개의 기능 수행 => 재사용, 반복 기능을 제거
	// 메소드 여러 개가 반복이 많은 경우 => 클래스 ==> jar
	/*
	 *   예약
	 *   ---
	 *   게스트하우스 1
	...
	public List<FoodVO> foodListData(int page)
	{
		...
		try
		{
			...
			while(rs.next()) // 첫번째 ROW부터 읽어서 마지막까지 읽음
			{
	...
	public FoodVO foodDetailData(int fno)
	{
		...
		try
		{
			...
			// SELECT => 실행된 데이터 결과를 읽어옴 => executeQuery
			// 실제 데이터를 가지고 옴
