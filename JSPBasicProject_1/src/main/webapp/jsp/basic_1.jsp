...
<%--
	...
	4. 자바 코딩
	    ...
	    = 출력문(표현식): 화면 출력 => out.write()
	      <%= %>
	5. 내장 객체: 9개 => 미리 객체를 생성한 후 사용
	    *** = request: 사용자의 요청 정보를 저장하고 있음
	...
	9. MVC
	   ...
	   1) 웹 동작: request(요청), response(응답)
	      ...
	      브라우저              |                          | Tomcat
	                       요청받음                            |
	                       파일찾기 ==> Servlet/JSP        JSP/Servlet
	                       => HTML/XML/JSON                 |
	                          (자동처리)                    HTML로 변환
	                       <===========                     |
	                        HTML을 브라우저로 전송
	                        => 번역된 내용: 소스보기
	ㅇ     19page 요청/응답 => 네트워크(C/S)
	...
	client ========== 객체생성
	  ...
	  1) 서블릿
	    ...
	    => JSP의 장점
	       ...
	       2) HTML/Java가 나뉘어져 있음
	       3) 디자인과 소스 코딩을 나눠서 작업할 수 있음
	       4) 단순함 => 데이터베이스 연동 => for/if
	       5) = 자바가 먼저 수행(데이터를 읽어옴) => HTML을 이용해서 출력
	       6) HTML => out.write("<html>")
	                  ========= 생략
	                  <html>태그가 단독으로 수행
	    => 서블릿은 클래스로 되어 있음
	       JSP는 클래스가 아니라 메소드 영역
	       =========================
 ...
 <%--
    ...
    3. .html, .css, .json => 웹서버 자체에서 처리 후 브라우저로 전송
       => jsp, servlet... 번역 불가
          ------------
               |
          번역을 요청 => 톰캣(resin, 제우스, 웹 로직, 웹 스페어...)
                         ================= WAS
          ...
          3) 자바 실행
             =======
                | 브라우저에 읽어 갈 메모리에 html을 출력해 줌
                  =================== 출력 스트림
          4) 출력된 메모리로부터 html을 한 줄씩 읽어서 브라우저 출력
                           ================ 인터프리터
                           
    71page => jsp의 생명주기
    => 자동 호출
       _jspInit() => 초기화(web.xml 등록 시 읽어감)
           |
