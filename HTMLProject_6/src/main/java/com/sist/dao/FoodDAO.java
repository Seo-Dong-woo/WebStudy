...
import java.util.*;
import java.sql.*; // Conection, ResultSet, PreparedStatement
import javax.sql.*; // 데이터베이스 저옵 => DataSource
 ...
 *               selectList() => SQL
 */
// JDBC => DBCP => ORM(2차 프로젝트: MyBatis/3차프로젝트: JPA)
//         ==== 1차 프로젝트
/*
 ...
 *      => Connection을 몇 개 설정 => maxIdle
 *         => 인원이 많아지면 어떻게 할 것인지 => maxActive
 *         => 반환 => 재사용 => 기다리는 시간 => maxWait => 음수면 무한정
 *   동작 => 생성된 Connection 객체를 POOL에 저장
 *                              ----- 저장 장소
 *   1. 미리 생성된 Connection 객체를 POOL안에서 얻어옴
 *      => POOL(JNDI) => 이름으로 탐색기를 만들어 놓은 것
 *             Java Naming Directoy Interface
 ...
 *   2. conn 사용
 *   3. conn 사용 후 반환 => POOL
 *   => 1. Connection의 생성 횟수를 줄일 수 있음(관리)
 *   => 2. 오라클 연결 시간을 줄임
