 ...
 *   2) init(): 초기화 => 생성자 대신
 *   3) service(): Thread => 사용자가 요청 시마다 호출되는 메소드
 *      =========
 *       => GET/POST => 동시처리
 *       => === ====
 *                | do Post => 데이터 전송
 *           | do Get => default(화면 UI)
 *   4) destroy()
 *   =================================================
 *    => JSP
...
public class CategoryServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// response안에는 클라이언트의 IP를 저장하고 있음
		// HTML을 메모리에 저장하면 => 브라우저에서 읽어감
		// 웹 서버는 이미 제작이 되어 있음
		// ----- CBD => 이미 기능이 제작 => 조립
		// 요청 값 받기
		...
		/*
		 *   bootstrap => 모든 내용 ==> close속성
		 *   CSS의 기본
		 *   1. * => 전체
		 *   2. 태그명
		 *   3. id ===> 중복이 없는 태그(한 개만 변경)
		 *      #id명
		 *   4. class ===> 중복이 있는 태그(여러 개를 동시 변경)
		 *      .class명
		 */
